...
function Playlist() {
	...
	var generateWeightedPlaylist = function (playlist) {
		...
		$.each(wheightedPlaylist, function (key, value) {
			...
			if (value.repetitionFrequency < 2)
				...
				else processedPlayList[value.repetitionFrequency] = new Array(value);
	...
	var separatePlaylistForWeightedPlay = function (playlist) {
		...
		playlist = createWeightedPlaylist(playlist)
	...
	console.log(separatePlaylistForWeightedPlay(jsonPlaylist))
}
