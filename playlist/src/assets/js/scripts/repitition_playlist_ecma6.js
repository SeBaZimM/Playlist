...
const Playlist = () => {
	...
	const addPlaylistElement = (mediaFile, duration, areaid, transition) => {
		let element = {
			mediaFile: mediaFile,
			duration: duration,
			areaid: areaid,
	...
	const createWeightedPlaylist = (playlist) => {

		typeof playlist === 'string' ? 
			playlist = JSON.parse(playlist) : 
			playlist = playlist;
		const processedPlaylist = playlist[Object.keys(playlist)];
		let finalPlaylist = [];
		let unweightList = {},
			weightList = {};

		try {
			const separatedLists = separatePlaylistForWeightedPlay(processedPlaylist);
		...
		$.each(unweightList, (element_key, element_unweightList) => {
			$.each(element_unweightList, (key, element) => {
				$.each(weightList, (weight, wList) => {
					if (key % weight === 0) {
						$.each(wList, (pElementKey, presentationElement) => {
	...
	const separatePlaylistForWeightedPlay = (playlist) => {

		let weightedList = {};
		let weightedListLength = 0;
		let unweightedList = {};
		let unweightedListLength = 0;

		$.each(playlist, (key, element) => {
	...
	const playList = createWeightedPlaylist(jsonPlaylist);
	let htmlScreen = document.getElementById('playlist');
	let element = 0,
		index_time = 0;
	let time = [];

	$.each(playList, (key, val) => time.push(val.duration * 1000));
